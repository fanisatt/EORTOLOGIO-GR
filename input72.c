...
/*
int input(char *str, int *pxl,int *pyt,int *pxr,int *pyb,int orio,short int arped)
{
SDL_Cursor* cursor;
register int i ,f ;
...
unsigned int time_metr ,ektos ,neg ;
...
mem_bar=malloc(size);
if (mem_bar==NULL) {printf ("Memory Problem") ; free (mem1); return (0); }
  ...
  if (vam[0]) {                              // βάφω 
    ...
    if(sdlev.button.button==SDL_BUTTON_LEFT) {   
        ...
        if (mx>xep && mx<xet && my>yb && my<yet) { // Choose ok ,flag , change dimensions
            ...
            switch (diaf1) {
                ...
                case 3: // (FYGE (Return) me pontiki)
                    ...
                    free(mem_find); free(mem_fonter); free(mem_coler);free(mem_link);
                    langex = (us_flag) ? 1 : 2 ;
                    cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);    SDL_SetCursor(cursor);refresh(); diaf1=ar; ar=0; undo=0; undo_firfile=1; undo_lasfile=0 ; cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; return diaf1;
        ...
        if (coler) {
                    ...
                    if(diak) {putimage(x,y,mem_cu,0); refresh(); diak=0;}
                    for (f=0 ; f<3 ; f++) { proxchxr[f]=chxr[f] ; proxbkxr[f]=bkxr[f] ; proxslxr[f]=slxr[f] ; }  
                    inkualizer (proxchxr , proxbkxr , proxslxr) ;
                    i=0;
                    for (f=0 ; f<3 ; f++) {
        ...
        if (linker) {
            ...
            user_ans=smmenu_show(11, submenu, str1024, xl+37,yt-20);
            for (f=0 ; f<3 ; f++) { chxr[f] = proxchxr[f] ; bkxr[f] = proxbkxr[f] ; slxr[f] = proxslxr[f]; }
            if (user_ans<0) continue ;
            size=0;
...
DROMO:
                ...
                cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; if (tab) return -9; else return 0;
 ...
 // ΑΝΤΙΓΡΑΦΩ ΟΣΑ ΕΠΙΤΡΕΠΕΤΑΙ ΑΠΟ ΤΟ SYSTEM CLIPBOARD αναλόγως του insert //
       ...
       y1= pos[a][1] ;
        ...
        break;
                    ...
                    free(mem_bar); 
                    if (keyb_gr) free(mem_flgr); 
                    free(mem_rmenu); free(mem_arrs); free(mem_info);free(mem_find); free(mem_fonter); free(mem_coler); free(mem_link);
                    str[ar]=255;
                    if (inp_mem_ghost) { free(inp_mem_ghost); inp_mem_ghost=0; }
                    langex = (us_flag) ? 1 : 2 ;
                    cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);    SDL_SetCursor(cursor);refresh(); diaf1=ar; ar=0; undo=0; undo_firfile=1; undo_lasfile=0 ; cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; return diaf1;
...
if (!SDL_PollEvent(&sdlev)) continue;
    ...
    if (mx>xl && mx<xl+18 && my>yt-21 && my<yt) { // Στην κεφαλίδα
        ...
        if (!marrs) {
            ...
            marrs=1;
            putNOTicon(xl,yt-20,mem_arrs);
            if (keyb_gr) show_help("Μετακίνηση Παραθύρου", xl , yt+10 , &hlpxt , &hlpyt) ;
            else show_help("Move Window", xl, yt+10, &hlpxt , &hlpyt) ;
            continue ;}
            else continue ;
        }
    ...
    if (mx>xr-18 && mx<xr+1 && my>yt-21 && my<yt) {  // Info Button
        ...
        if (!inp_info) {
          ...
          inp_info=1;
          putNOTicon(xr-18,yt-20,mem_info);
          if (keyb_gr) show_help("Γενικά στοιχεία για το κείμενο", xr-18 , yt+10 , &hlpxt , &hlpyt) ;
          else show_help("General info for the text", xr-18, yt+10, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    ...
    if (mx>xr-36 && mx<xr-18 && my>yt-21 && my<yt) {  // Search Button
        ...
        show_help (str1024 , xr-18, yt+10, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    ...
    if (mx>xr-54 && mx<xr-36 && my>yt-21 && my<yt) {  // Fonts Button
        ...
        if (!fonter) {
          ...
          fonter=1;
          putNOTicon(xr-54,yt-20,mem_fonter);
          if (keyb_gr) show_help("Επιλογή Γραμματοσειράς (προσωρινή χρήση)", xr-54 , yt+10 , &hlpxt , &hlpyt) ;
          else show_help("Choose Font (temporary use)", xr-54, yt+10, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    ...
    if (fonter) {fonter=0 ; puticon(xr-54,yt-20,mem_fonter,255); hide_help(hlpxt , hlpyt);}
        ...
        if (!linker) {
          ...
          linker=1;
          putNOTicon(xl+19,yt-20,mem_link);
          if (keyb_gr) show_help("Επιλεγμένο κείμενο ως διαδικτυακός σύνδεσμος", xl+19 , yt+10 , &hlpxt , &hlpyt) ;
          else show_help("Selected text as a WEB link", xl+19 , yt+10 , &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
